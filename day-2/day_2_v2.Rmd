...
childrens_lit <- read_delim("data/childrens_lit.csv", delim = "\t") %>% 
  na.omit() %>% select(-X1)
...
cl_custom_stop_words <- 
  bind_rows(tibble(word = c("a m", "a ve", "de", "en"), 
                   lexicon , "em", "tom"= c("custom")), 
                   stop_words)
...
beta_wide %>% 
ggplot(beta_wide, aes(y = fct_top_n(20, abs(log_ratio)) %>% reorder(atio),
                      x = log_ratio)) +
   geom_col(show.legend = FALSE) +
  labs(y = "",
